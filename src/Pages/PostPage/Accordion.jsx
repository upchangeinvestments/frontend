    ...
    return (
        <div className="w-full">
            {DropDownData.map((data, index) => (
                <Accordion className="my-4 rounded-lg bg-[#F3F3F3] border-[1px] border-solid border-gray-600 py-0" key={index} open={open === index}>
                    <AccordionHeader className="w-full text-lg font-normal px-4 rounded-t-lg flex-row border-none pt-2 pb-1" onClick={() => handleOpen(index)}>
                        <div className="vsm:w-3/4 md:w-1/4 CustomizeFontH">
                            {data.title}
                        </div>
                        <div className="vsm:w-1/4 vsm:-mr-6 md:mr-0 md:w-3/4 grow vsm:text-xs lg:text-base flex vsm:justify-end lg:justify-between">
                            <span className={`vsm:hidden ${open == index ? 'md:hidden' : 'lg:block'}`}>{data.content}</span>
                            <span className={`flex items-center justify-center  ${open == index ? 'hidden' : 'block'} whitespace-nowrap`}>
                                <p>see more</p>
                                <svg
