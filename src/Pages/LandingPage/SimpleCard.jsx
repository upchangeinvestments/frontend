...
function SimpleCard({ heading, items }) {
    return (
        <div className='flex flex-col items-center rounded-lg justify-center p-4 bg-white w-80 bg-white/50 backdrop-blur-[100px]'
            style={{
                ...
                {items.map((element, index) => (
                    <li className="ms-6" key={index}>
                        <span className="absolute flex items-center justify-center w-2 h-2 rounded-full -left-1 ">
                            <div className="absolute top-1"><FaCircleCheck size="20px" className="text-[#9747FF] " /></div>
                        </span>
                        <p class="mb-4 text-base font-normal ">{element}</p>
                    </li>
                ))}
            </ol>
        </div>
